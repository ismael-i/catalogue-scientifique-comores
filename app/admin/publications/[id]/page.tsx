...
import {
  ArrowLeft, Pencil, Trash2, FileText, Users, FlaskConical,
  Calendar, Tag, Loader2, AlertCircle, ExternalLink, BookOpen
} from "lucide-react"
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="bg-red-50 border border-red-200 text-red-700 p-4 rounded-2xl flex gap-3">
          ...
          <p>{error || "Publication non trouvée"}</p>
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex items-center justify-between mb-6">
        <Link href="/admin/publications" className="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700">
          <ArrowLeft className="w-4 h-4" /> Retour à la liste
        </Link>
        <div className="flex gap-2">
          <Link href={`/admin/publications/${id}/modifier`} className="inline-flex items-center gap-2 px-4 py-2 bg-amber-50 text-amber-700 text-sm font-medium rounded-xl border border-amber-200 hover:bg-amber-100">
            <Pencil className="w-4 h-4" /> Modifier
          </Link>
          <button onClick={() => setShowDeleteModal(true)} className="inline-flex items-center gap-2 px-4 py-2 bg-red-50 text-red-700 text-sm font-medium rounded-xl border border-red-200 hover:bg-red-100">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sm:p-8 mb-6">
        <div className="flex items-center gap-2 mb-2">
          <span className="text-[10px] font-semibold px-2.5 py-0.5 rounded-full bg-blue-50 text-blue-600 border border-blue-100">{publication.type.replace("_", " ")}</span>
          <span className="text-[10px] font-semibold px-2.5 py-0.5 rounded-full bg-gray-50 text-gray-600 border border-gray-200">{publication.domain}</span>
        </div>
          ...
          <div className="mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              {publication.keywords.map(kw => (
                <span key={kw.id} className="px-3 py-1 bg-blue-50 text-blue-700 text-xs rounded-full border border-blue-100">{kw.keyword}</span>
              ))}
            </div>
          </div>
        )}
        ...
        {publication.pdfUrl && (
          <div>
            <a href={getFileUrl(publication.pdfUrl)} target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center gap-2 px-4 py-2 bg-red-50 text-red-700 rounded-xl hover:bg-red-100 transition-colors text-sm font-medium">
